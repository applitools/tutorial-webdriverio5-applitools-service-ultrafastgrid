...
const parseUserAgent = require('./user-agent')
...
class Driver {
  static specialize(spec) {
    return class SpecializedDriver extends Driver {
      static get spec() {
        return spec
      }
      get spec() {
  ...
  static isDriver(driver) {
    return driver instanceof Driver || this.spec.isDriver(driver)
  }
   ...
   */
  static get spec() {
    throw new TypeError('The class is not specialized. Create a specialize Driver first')
  }
  /** @type {SpecDriver<TDriver, TContext, TElement, TSelector>} */
  get spec() {
    throw new TypeError('The class is not specialized. Create a specialize Driver first')
  ...
  async init() {
    if (this.spec.getDriverInfo) {
      ...
      if (!this._isNative) {
        ...
        this._userAgent = parseUserAgent(this._userAgentString, true)
      }
    }
    if (this._userAgent) {
      if (!this._isMobile) this._isMobile = ['iOS', 'Android'].includes(this._userAgent.os)
      if (!this._platformName) this._platformName = this._userAgent.os
      if (!this._platformVersion) this._platformVersion = this._userAgent.osMajorVersion
      if (!this._browserName) this._browserName = this._userAgent.browser
      if (!this._browserVersion) this._browserVersion = this._userAgent.browserMajorVersion
    }
  ...
  async refreshContexts() {
    ...
    let contextInfo = await scripts.getContextInfo(this._logger, this)
    ...
    while (!contextInfo.isRoot) {
      ...
      contextInfo = await scripts.getContextInfo(this._logger, this)
    }
    this._currentContext = this._mainContext
  ...
  async findChildContext(contextInfo) {
    ...
    const framesInfo = await scripts.getChildFramesInfo(this._logger, this)
  ...
  async switchToMainContext() {
    ...
    this._logger.verbose('Driver.switchToMainContext()')
  ...
  async switchToParentContext(elevation = 1) {
    ...
    this._logger.verbose(`Driver.switchToParentContext(${elevation})`)
  ...
  async switchToChildContext(...references) {
    if (this._isNative) return
    this._logger.verbose('Driver.childContext()')
  ...
  async getViewportSize() {
    const size = this.spec.getViewportSize
      ...
      : await scripts.getViewportSize(this._logger, this._mainContext)
    return size
  }
  /**
  ...
  async setViewportSize(size) {
    return this.spec.setViewportSize
      ...
      : scripts.setViewportSize(this._logger, this._mainContext, size)
  }
  /**
  ...
  async getPixelRatio() {
    if (this._isNative) {
      ...
      return scripts.getPixelRatio(this._logger, this._currentContext)
    }
  }
  /**
